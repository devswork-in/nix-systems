...
def get_battery_info():
    try:
        ...
        if state in ["charging", "fully-charged"]:
             icon = f"{icons[idx]}  <span size='small'></span>"
        else:
             icon = icons[idx]
...
def main():
    ...
    # Monitor upower events for near-instant updates
    # This process will stay alive and Waybar will read its stdout stream
    ...
    try:
        for _ in proc.stdout:
            # Whenever upower monitor outputs a line, refresh our info
            print(json.dumps(get_battery_info()), flush=True)
