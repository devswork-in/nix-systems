...
, desktopActions ? null, mimeType ? null }:
...
in pkgs.appimageTools.wrapType2 {
  ...
  binName =
    pname; # Explicitly set the executable name to match pname for consistency

  # Audio libraries for PipeWire/PulseAudio compatibility
  extraPkgs = pkgs:
    with pkgs; [
      libpulseaudio
      alsa-lib
          ...
          if desktopActions != null && desktopActions != "" then
            let
              ...
              actionNames = map (action:
                let parts = pkgs.lib.splitString ":" action;
                in pkgs.lib.elemAt parts 0) actions;
